...
(function() {
  ...
  $.api.notifiers = api = {
    init: function() {
      var $destroy, $events, $form, $formContainer, $formToggler, $notifiers, dispatcher, notifierId;
      if ($.api.action === 'index') {
        ...
        $formToggler.bind('click', function(event) {
          event.preventDefault();
          $formContainer.slideToggle('fast');
          $(this).toggleClass('active');
          return $('#notifier_path').autocomplete({
            source: 'notifiers',
            minLength: 2
          });
        });
        $form.bind('ajax:complete', function(event, xhr, status) {
          var $this, attribute, errors, input, response, _ref, _results;
          $this = $(this);
          ...
          if (status === 'success') {
            ...
            $formContainer.slideToggle('fast');
            return $formToggler.removeClass('active');
          } else {
            ...
            for (attribute in _ref) {
              ...
              _results.push($.api.utils.appendErrorsTo(input, errors));
            }
            return _results;
          }
        }).bind('ajax:beforeSend', function() {
          return $.api.utils.removeErrorsFrom($(this));
        });
        $notifiers.on('ajax:beforeSend', $destroy, function() {
      ...
      if ($.api.action === 'show') {
        ...
        return dispatcher.addEventListener('fileCreated', function(event) {
          var eventData;
          eventData = $.parseJSON(event.data);
          return $events.html(eventData.events);
        });
      }
    }
  };
